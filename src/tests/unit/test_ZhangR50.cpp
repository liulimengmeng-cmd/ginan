...
BOOST_AUTO_TEST_CASE(r50_ratio_policy_retains_structural_rejections)
{
    unsetenv("ZHANG_R50_RATIO_ONLY");
    BOOST_CHECK(!zhangR50StatisticalAccept(false));
    setenv("ZHANG_R50_RATIO_ONLY","1",1);
    BOOST_CHECK(zhangR50StatisticalAccept(false));
    BOOST_CHECK(!zhangR50StatisticalReject(true));
    ...
    BOOST_CHECK(zhangR50StatisticalAccept(high.nis<=high.threshold));
    ...
    proof->id="R50_TEST";proof->originalStatement="x=0";proof->observationProvenance="TEST";proof->conditionalFailureBound=1;
