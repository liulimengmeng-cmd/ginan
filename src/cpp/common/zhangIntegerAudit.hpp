...
)
{
    ...
    if (removed.empty())
    {
        ...
        ZhangExactRowHnf hnf = zhangExactRowHermiteNormalForm(rows, values, trackRowTransform);
        result.basis = std::move(hnf.basis);
        result.values = std::move(hnf.values);
        result.rowTransform = std::move(hnf.rowTransform);
    ...
    if (trackRowTransform)
    {
        // Compose transformations already computed by the kernel and HNF;
...
struct ZhangExactCanonicalAffineProjection
{
    bool              valid = false;
    ZhangExactMatrix  basis;
    ZhangExactVector  values;
    int               canonicalInputRank = 0;
    int               directlyMappableRank = 0;
    int               recoveredCombinationRank = 0;
    int               touchedRows = 0;
};
 ...
 *
 * canonicalToCurrent[c] is the current output coordinate for canonical
 * direction c, or -1 when that direction is currently unavailable.  The
 * integer kernel is computed across the complete input family first.  Thus two
 * individually unmappable rows may still produce a valid current relation when
 * their unavailable coefficients cancel exactly. */
inline ZhangExactCanonicalAffineProjection
zhangExactProjectCanonicalAffineLattice(
    const ZhangExactMatrix& canonicalRows,
    const ZhangExactVector& canonicalValues,
    const std::vector<int>& canonicalToCurrent,
    int currentDimension)
{
    ZhangExactCanonicalAffineProjection result;
    if (canonicalRows.empty() || canonicalRows.size() != canonicalValues.size() ||
        currentDimension <= 0 || canonicalToCurrent.empty())
    {
        return result;
    }
    std::vector<bool> available(canonicalToCurrent.size(), false);
    std::vector<int> currentOwners(currentDimension, -1);
    for (int canonical = 0;
         canonical < static_cast<int>(canonicalToCurrent.size()); canonical++)
    {
        const int current = canonicalToCurrent[canonical];
        if (current < 0) continue;
        if (current >= currentDimension || currentOwners[current] >= 0)
            return result;
        currentOwners[current] = canonical;
        available[canonical] = true;
    }
    if (std::any_of(currentOwners.begin(), currentOwners.end(),
        [](const int owner) { return owner < 0; }))
    {
        return result;
    }
    for (const auto& row : canonicalRows)
        if (row.size() != canonicalToCurrent.size()) return result;

    const auto inputHnf = zhangExactRowHermiteNormalForm(
        canonicalRows, canonicalValues);
    if (!inputHnf.consistent) return result;
    result.canonicalInputRank = inputHnf.basis.size();

    ZhangExactMatrix directRows;
    ZhangExactVector directValues;
    for (int row = 0; row < static_cast<int>(canonicalRows.size()); row++)
    {
        bool direct = true;
        for (int column = 0; column < static_cast<int>(available.size()); column++)
            direct &= available[column] || canonicalRows[row][column] == 0;
        if (direct)
        {
            directRows.push_back(canonicalRows[row]);
            directValues.push_back(canonicalValues[row]);
        }
    }
    const auto directHnf = zhangExactRowHermiteNormalForm(
        directRows, directValues);
    if (!directHnf.consistent) return result;
    result.directlyMappableRank = directHnf.basis.size();

    const auto surviving = zhangExactSurvivingLattice(
        canonicalRows, canonicalValues, available);
    if (!surviving.consistent) return result;
    ...
    for (const auto& compact : surviving.basis)
    {
        ...
        int compactColumn = 0;
        for (int canonical = 0;
             canonical < static_cast<int>(available.size()); canonical++)
        {
            if (!available[canonical]) continue;
            if (compactColumn >= static_cast<int>(compact.size())) return result;
            projected[canonicalToCurrent[canonical]] = compact[compactColumn++];
        }
        if (compactColumn != static_cast<int>(compact.size())) return result;
        projectedRows.push_back(std::move(projected));
    }
    const auto projectedHnf = zhangExactRowHermiteNormalForm(
        projectedRows, surviving.values);
    if (!projectedHnf.consistent) return result;
    result.basis = projectedHnf.basis;
    result.values = projectedHnf.values;
    result.recoveredCombinationRank = std::max(0,
        static_cast<int>(result.basis.size()) - result.directlyMappableRank);
    result.valid = true;
    return result;
}
...
/** Exact membership for many targets in the same row lattice.
 *
 * The Smith-style diagonalisation of rows^T is the expensive part.  Applying
 * its row operations to every target column at once is algebraically
 * identical to repeated zhangIntegerRowLatticeContains() calls, but performs
 * the multiprecision diagonalisation only once. */
inline std::vector<ZhangIntegerLatticeMembership>
zhangIntegerRowLatticeContainsBatch(
    const ZhangExactMatrix& rows,
    const ZhangExactMatrix& targets)
{
    std::vector<ZhangIntegerLatticeMembership> results(targets.size());
    if (targets.empty()) return results;
    const std::size_t dimension = targets.front().size();
    const std::size_t rowCount = rows.size();
    for (const auto& target : targets)
        if (target.size() != dimension) return results;
    for (const auto& row : rows)
        if (row.size() != dimension) return results;
    if (dimension == 0)
    {
        for (auto& result : results) result.contained = true;
        return results;
    }
    ...
    for (std::size_t column = 0; column < dimension; column++)
        matrix[column][row] = rows[row][column];
    ZhangExactMatrix transformed = zhangExactZeroMatrix(
        dimension, targets.size());
    for (std::size_t target = 0; target < targets.size(); target++)
    for (std::size_t row = 0; row < dimension; row++)
        transformed[row][target] = targets[target][row];
    ZhangExactMatrix rightTransform = zhangExactIdentityMatrix(rowCount);
    ...
    {
        for (std::size_t column = 0; column < rowCount; column++)
            matrix[destination][column] +=
                multiplier * matrix[source][column];
        for (std::size_t target = 0; target < targets.size(); target++)
            transformed[destination][target] +=
                multiplier * transformed[source][target];
    };
    auto swapColumns = [&](std::size_t left, std::size_t right)
    {
        for (auto& row : matrix) std::swap(row[left], row[right]);
        for (auto& row : rightTransform) std::swap(row[left], row[right]);
    };
    auto addColumnMultiple = [&](std::size_t destination,
                                 std::size_t source,
                                 const ZhangExactInteger& multiplier)
    ...
    while (pivot < dimension && pivot < rowCount)
    {
        ...
        if (selectedRow == dimension) break;
        swapRows(pivot, selectedRow);
        swapColumns(pivot, selectedColumn);
        ...
        while (true)
        {
            ...
            for (std::size_t row = pivot + 1; row < dimension; row++)
            {
                if (matrix[row][pivot] == 0) continue;
                const ZhangExactInteger quotient =
                    matrix[row][pivot] / matrix[pivot][pivot];
                addRowMultiple(row, pivot, -quotient);
                if (matrix[row][pivot] != 0) swapRows(row, pivot);
                restart = true;
                break;
            }
            if (restart) continue;
            ...
            {
                if (matrix[pivot][column] == 0) continue;
                const ZhangExactInteger quotient =
                    matrix[pivot][column] / matrix[pivot][pivot];
                addColumnMultiple(column, pivot, -quotient);
                if (matrix[pivot][column] != 0)
            ...
            for (std::size_t row = pivot + 1;
                 row < dimension && offendingRow == dimension; row++)
            for (std::size_t column = pivot + 1;
                 column < rowCount; column++)
            {
                if (matrix[row][column] % matrix[pivot][pivot] != 0)
    ...
    std::vector<ZhangExactInteger> invariants;
    for (std::size_t row = 0; row < pivot; row++)
        invariants.push_back(matrix[row][row]);
    for (std::size_t target = 0; target < targets.size(); target++)
    {
        auto& result = results[target];
        result.rank = static_cast<int>(pivot);
        result.smithInvariants = invariants;
        result.contained = true;
        for (std::size_t row = 0; row < pivot; row++)
            if (transformed[row][target] % matrix[row][row] != 0)
                result.contained = false;
        for (std::size_t row = pivot; row < dimension; row++)
            if (transformed[row][target] != 0)
                result.contained = false;
        if (!result.contained) continue;
        ZhangExactVector diagonalCoordinates(rowCount);
        for (std::size_t row = 0; row < pivot; row++)
            diagonalCoordinates[row] =
                transformed[row][target] / matrix[row][row];
        result.combination = zhangExactMatrixTimesColumn(
            rightTransform, diagonalCoordinates);
    }
    return results;
}
...
)
{
    ...
    targets.reserve(3 * signalDimension);
    for (std::size_t index = 0; index < signalDimension; index++)
    {
        ...
        targets.push_back(std::move(l1));
        targets.push_back(std::move(l2));
        targets.push_back(std::move(wideLane));
    }
    const auto memberships = zhangIntegerRowLatticeContainsBatch(
        heldRows, targets);
    for (std::size_t index = 0; index < signalDimension; index++)
    {
        validity.l1 &= memberships[3 * index].contained;
        validity.l2 &= memberships[3 * index + 1].contained;
        validity.wideLane &= memberships[3 * index + 2].contained;
    }
    return validity;
}
...
{
    ZhangExactMatrix             matrix;
    // Canonical A_(m-1) satellite-difference lattice in sorted satellite
    // coordinates.  Unlike matrix (the exact pullback into the current
    // chord coordinates), this object is independent of the backend tree,
    // product tree, receiver core and reference satellite.
    ZhangExactMatrix             canonicalMatrix;
    std::vector<SatSys>          canonicalSatellites;
    std::string                  canonicalHnf;
    std::vector<ZhangExactInteger> canonicalSmithInvariants;
    int                          canonicalRank = 0;
    bool                         canonicalPrimitive = false;
    std::vector<ZhangGraphEdge>  currentChords;
    std::vector<SatSys>          targetSatellites;
    SatSys                       referenceSatellite;
    // A persistent product tree is a representation, not an authoritative
    // observation.  When one of its former edges leaves the current graph we
    // rebuild an equivalent rooted tree using only currently represented
    // edges, then run the original subset/audit checks on that rebuilt tree.
...
/** Build the canonical satellite product target
 *      T = {q in Z^m : 1^T q = 0}.
 *
 * Rows are initially written as e_s-e_ref, but the returned HNF is the
 * canonical object.  Consequently a different reference satellite changes
 * only the generating rows, never the represented integer lattice.
 */
inline ZhangSatelliteProductTarget zhangCanonicalSatelliteProductTarget(
    const std::set<SatSys>& satellites,
    const SatSys& requestedReference = SatSys())
{
    ZhangSatelliteProductTarget result;
    result.canonicalSatellites.assign(satellites.begin(), satellites.end());
    if (result.canonicalSatellites.empty())
    {
        result.failureReason = "canonical_product_target_has_no_satellite";
        return result;
    }
    result.referenceSatellite = requestedReference.prn == 0
        ? result.canonicalSatellites.front() : requestedReference;
    auto reference = std::find(
        result.canonicalSatellites.begin(),
        result.canonicalSatellites.end(),
        result.referenceSatellite);
    if (reference == result.canonicalSatellites.end())
    {
        result.failureReason = "reference_satellite_not_in_component";
        return result;
    }
    const std::size_t referenceColumn = std::distance(
        result.canonicalSatellites.begin(), reference);
    for (std::size_t column = 0;
         column < result.canonicalSatellites.size(); column++)
    {
        if (column == referenceColumn) continue;
        ZhangExactVector row(result.canonicalSatellites.size());
        row[column] = 1;
        row[referenceColumn] = -1;
        result.canonicalMatrix.push_back(std::move(row));
    }
    const auto hnf = zhangExactRowHermiteNormalForm(result.canonicalMatrix);
    if (!hnf.consistent)
    {
        result.failureReason = "canonical_product_target_hnf_failed";
        return result;
    }
    result.canonicalMatrix = hnf.basis;
    result.canonicalRank = static_cast<int>(hnf.basis.size());
    result.canonicalHnf = zhangExactMatrixFingerprint(hnf.basis);
    const auto smith = zhangIntegerRowLatticeContains(
        hnf.basis, ZhangExactVector(result.canonicalSatellites.size()));
    result.canonicalSmithInvariants = smith.smithInvariants;
    result.canonicalPrimitive = result.canonicalRank ==
        static_cast<int>(result.canonicalSatellites.size()) - 1;
    for (const auto& invariant : result.canonicalSmithInvariants)
        result.canonicalPrimitive &= zhangExactAbs(invariant) == 1;
    result.valid = result.canonicalPrimitive;
    if (!result.valid)
        result.failureReason = "canonical_product_target_not_primitive";
    return result;
}
...
)
{
    ...
    ZhangGraphBasis effectiveProductBasis = productBasis;
    std::set_difference(
        productBasis.treeEdges.begin(), productBasis.treeEdges.end(),
        currentBasis.edges.begin(), currentBasis.edges.end(),
        std::back_inserter(result.staleProductTreeEdges));
    if (!result.staleProductTreeEdges.empty())
    {
        effectiveProductBasis = zhangBuildRootedProductTree(
            currentBasis.edges,
            currentBasis.rootReceiver,
            productBasis.treeEdges,
            {},
            currentBasis.edges);
        result.productTreeTransported = true;
        if (!effectiveProductBasis.connected ||
            effectiveProductBasis.rootReceiver != currentBasis.rootReceiver ||
            effectiveProductBasis.satellites != currentBasis.satellites ||
            !std::includes(
                currentBasis.edges.begin(), currentBasis.edges.end(),
                effectiveProductBasis.treeEdges.begin(),
                effectiveProductBasis.treeEdges.end()))
        {
            result.failureReason =
                "product_tree_transport_to_current_graph_failed";
            return result;
        }
    }
    ...
    ZhangCanonicalIntegerAudit product =
        zhangCanonicalIntegerAudit(effectiveProductBasis);
    ...
    if (effectiveProductBasis.satellites.empty())
    {
        result.failureReason = "product_tree_has_no_satellite";
        return result;
    }
    const auto canonical = zhangCanonicalSatelliteProductTarget(
        effectiveProductBasis.satellites, requestedReference);
    if (!canonical.valid)
    {
        result.failureReason = canonical.failureReason;
        return result;
    }
    result.canonicalMatrix = canonical.canonicalMatrix;
    result.canonicalSatellites = canonical.canonicalSatellites;
    result.canonicalHnf = canonical.canonicalHnf;
    result.canonicalSmithInvariants = canonical.canonicalSmithInvariants;
    result.canonicalRank = canonical.canonicalRank;
    result.canonicalPrimitive = canonical.canonicalPrimitive;
    result.referenceSatellite = requestedReference;
    if (result.referenceSatellite.prn == 0)
    {
        result.referenceSatellite = *effectiveProductBasis.satellites.begin();
    }
    if (effectiveProductBasis.satellites.find(result.referenceSatellite) ==
        effectiveProductBasis.satellites.end())
    {
        result.failureReason = "reference_satellite_not_in_component";
        return result;
    }
    ...
    const std::size_t satelliteOffset =
        effectiveProductBasis.receivers.size() - 1;
    map<SatSys, std::size_t> satelliteRow;
    std::size_t row = satelliteOffset;
    for (const auto& satellite : effectiveProductBasis.satellites)
    {
