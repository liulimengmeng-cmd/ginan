...
inline void zhangFactorIdAppendBytes(
...
inline void zhangFactorIdAppendPod(std::uint64_t& hash, const Value& value)
{
	zhangFactorIdAppendBytes(hash, &value, sizeof(value));
}

inline void zhangFactorIdAppendString(
	std::uint64_t& hash, const std::string& value)
{
	const std::uint64_t size = value.size();
	zhangFactorIdAppendPod(hash, size);
	zhangFactorIdAppendBytes(hash, value.data(), value.size());
}

inline void zhangFactorIdAppendKey(
	std::uint64_t& hash, const ZhangCapturedStateKey& key)
{
	zhangFactorIdAppendPod(hash, key.type);
	const int satelliteSystem = static_cast<int>(key.satellite.sys);
	zhangFactorIdAppendPod(hash, satelliteSystem);
	zhangFactorIdAppendPod(hash, key.satellite.prn);
	zhangFactorIdAppendString(hash, key.receiver);
	zhangFactorIdAppendPod(hash, key.number);
}

inline void zhangFactorIdAppendSparse(
	std::uint64_t& hash, const SparseMatrix<double>& matrix)
{
	...
	zhangFactorIdAppendPod(hash, nonZeros);
	for (int outer = 0; outer < matrix.outerSize(); outer++)
	for (SparseMatrix<double>::InnerIterator entry(matrix, outer);
		 entry; ++entry)
	{
		const int row = entry.row();
		const int column = entry.col();
		const double value = entry.value();
		zhangFactorIdAppendPod(hash, row);
		zhangFactorIdAppendPod(hash, column);
		zhangFactorIdAppendPod(hash, value);
	}
}

inline void zhangFactorIdAppendVector(
	std::uint64_t& hash, const VectorXd& vector)
{
...
inline std::string zhangCapturedFactorIdentity(
	const ZhangCapturedFactorEvent& event)
{
	std::uint64_t hash = 1469598103934665603ULL;
	const int kind = static_cast<int>(event.kind);
	zhangFactorIdAppendPod(hash, kind);
	zhangFactorIdAppendPod(hash, event.beforeCommitSequence);
	zhangFactorIdAppendPod(hash, event.afterCommitSequence);
	zhangFactorIdAppendString(hash, event.label);
	for (const auto& key : event.sourceKeys)
	{
		zhangFactorIdAppendKey(hash, key);
	}
	for (const auto& key : event.destinationKeys)
	{
		zhangFactorIdAppendKey(hash, key);
	}
	for (const auto& key : event.observationKeys)
	{
		zhangFactorIdAppendKey(hash, key);
	}
	zhangFactorIdAppendSparse(hash, event.design);
	zhangFactorIdAppendSparse(hash, event.covariance);
	zhangFactorIdAppendVector(hash, event.rightHandSide);
...
struct ZhangFactorCaptureSummary
{
	...
	std::size_t uniqueMeasurementFactorIds = 0;
	std::size_t duplicateMeasurementFactorIds = 0;
	std::string factorChronologyHash;
...
/** Exact chronology identity of the accepted measurement-factor prefix used
 * by a fixed-lag marginal.
 *
 * `measurementSequences` are the immutable event sequence numbers assigned
 * when the accepted KF measurement block is captured.  State transitions and
 * coordinate transforms are deliberately excluded: they transport or weaken
 * information but are not new observational evidence.  The window identity
 * binds the sequence numbers to one raw-factor anchor, so a reset sequence can
 * never be mistaken for a continuation of the previous window. */
struct ZhangCapturedFactorEvidenceWindow
{
	bool valid = false;
	std::string identity;
	std::vector<std::size_t> measurementSequences;
	std::size_t measurementRows = 0;
...
public:
	void clear()
	{
		...
		maximumCommitMeanRelativeError = 0;
		maximumCommitCovarianceRelativeError = 0;
	...
	/** Compatibility entry point for synthetic/unit callers.  Production KF
	 * callbacks use the explicit authoritative before/after commit overload. */
	...
	{
		const std::uint64_t before = commitSequenceAnchored
			? lastCommitSequence : 0;
		return recordMeasurement(
			time, keys, priorMean, priorCovariance, measurement, label,
			posteriorMean, posteriorCovariance, before, before + 1);
	}
		...
		std::uint64_t afterCommitSequence)
	{
		...
		if (afterCommitSequence != beforeCommitSequence + 1
		 || (commitSequenceAnchored
			 && beforeCommitSequence != lastCommitSequence))
		{
			lastFailure = "AUTHORITATIVE_COMMIT_SEQUENCE_BREAK";
			return false;
		}
		// A rejected callback is a failed transaction.  Complete key-chain and
		// replay validation before mutating either raw square-root accumulator.
		if (anchored && !currentKeys.empty() && currentKeys != keys)
		{
		...
		event.beforeCommitSequence = beforeCommitSequence;
		event.afterCommitSequence = afterCommitSequence;
		event.label = label;
		...
		event.factorId = zhangCapturedFactorIdentity(event);
		...
		lastCommitSequence = afterCommitSequence;
		commitSequenceAnchored = true;
		trimFailClosed();
		return true;
	}
	...
	bool recordTransition(
		...
		const std::string& label)
	{
		const VectorXd affineOffset = VectorXd::Zero(transition.rows());
		const VectorXd committedMean = transition * replayMean;
		MatrixXd committedCovariance = transition * replayCovariance
			* transition.transpose() + processCovariance;
		committedCovariance = 0.5
			* (committedCovariance + committedCovariance.transpose());
		const std::uint64_t before = lastCommitSequence;
		return recordTransition(
			time, source, destination, transition, processCovariance,
			affineOffset, label, committedMean, committedCovariance,
			before, before + 1);
	}
	...
	bool recordTransition(
		...
		const std::string& label,
		const VectorXd& committedMean,
		const MatrixXd& committedCovariance,
		std::uint64_t beforeCommitSequence,
		std::uint64_t afterCommitSequence)
	{
		if (!anchored)
		...
		if (source.empty() || destination.empty()
		 ...
		 || committedMean.size() != transition.rows()
		 || committedCovariance.rows() != transition.rows()
		 || committedCovariance.cols() != transition.rows()
		 || !processCovariance.allFinite() || !affineOffset.allFinite()
		 || !committedMean.allFinite() || !committedCovariance.allFinite())
		{
			lastFailure = "INVALID_TRANSITION_CAPTURE_OR_KEY_CHAIN";
			return false;
		}
		if (!commitSequenceAnchored
		 || afterCommitSequence != beforeCommitSequence + 1
		 || beforeCommitSequence != lastCommitSequence)
		{
			lastFailure = "AUTHORITATIVE_COMMIT_SEQUENCE_BREAK";
			return false;
		}
		const VectorXd predictedMean = transition * replayMean + affineOffset;
		...
		const double commitMeanError =
			(predictedMean - committedMean).norm()
			/ std::max(1.0, committedMean.norm());
		const double commitCovarianceError =
			(predictedCovariance - committedCovariance).norm()
			/ std::max(1.0, committedCovariance.norm());
		maximumCommitMeanRelativeError = std::max(
			maximumCommitMeanRelativeError, commitMeanError);
		maximumCommitCovarianceRelativeError = std::max(
			maximumCommitCovarianceRelativeError, commitCovarianceError);
		if (commitMeanError > replayTolerance
		 || commitCovarianceError > replayTolerance)
		{
			lastFailure = "AUTHORITATIVE_TRANSITION_COMMIT_MISMATCH";
			return false;
		}
		auto transportedTargets = lastTargets;
		if (!transportPersistentFunctionals(
				transportedTargets, source, destination, transition,
				&processCovariance, &affineOffset,
				"STATE_TRANSITION", false))
		{
			return false;
		}
		if (!incrementalRawSquareRoot.advance(
				MatrixXd(transition), processCovariance, affineOffset))
		{
			lastFailure = incrementalRawSquareRoot.summary().failureReason;
			return false;
		}
		...
		event.rightHandSide = affineOffset;
		event.factorId = zhangCapturedFactorIdentity(event);
		...
		replayMean = committedMean;
		replayCovariance = committedCovariance;
		lastCommitSequence = afterCommitSequence;
	...
	bool recordCoordinateTransform(
		...
		bool preserveUnrepresentablePersistentTargets = false)
	{
		const VectorXd affineOffset = VectorXd::Zero(transform.rows());
		const VectorXd committedMean = transform * replayMean;
		MatrixXd committedCovariance = transform * replayCovariance
			* transform.transpose();
		committedCovariance = 0.5
			* (committedCovariance + committedCovariance.transpose());
		const std::uint64_t before = lastCommitSequence;
		return recordCoordinateTransform(
			time, source, destination, transform, label,
			preserveUnrepresentablePersistentTargets, affineOffset,
			committedMean, committedCovariance, before, before + 1);
	}
	...
	bool recordCoordinateTransform(
		...
		const SparseMatrix<double>& transform,
		const std::string& label,
		bool preserveUnrepresentablePersistentTargets,
		const VectorXd& affineOffset,
		const VectorXd& committedMean,
		const MatrixXd& committedCovariance,
		std::uint64_t beforeCommitSequence,
		std::uint64_t afterCommitSequence)
	{
		...
		if (source.empty() || destination.empty()
		 ...
		 || (!currentKeys.empty() && currentKeys != source)
		 || affineOffset.size() != transform.rows()
		 || committedMean.size() != transform.rows()
		 || committedCovariance.rows() != transform.rows()
		 || committedCovariance.cols() != transform.rows()
		 || !affineOffset.allFinite() || !committedMean.allFinite()
		 || !committedCovariance.allFinite())
		{
		...
		const VectorXd transformedMean = transform * replayMean + affineOffset;
		...
		const double commitMeanError =
			(transformedMean - committedMean).norm()
			/ std::max(1.0, committedMean.norm());
		const double commitCovarianceError =
			(transformedCovariance - committedCovariance).norm()
			/ std::max(1.0, committedCovariance.norm());
		maximumCommitMeanRelativeError = std::max(
			maximumCommitMeanRelativeError, commitMeanError);
		maximumCommitCovarianceRelativeError = std::max(
	...
	{
		return persistentRawTargetWindow.targetDifferenceMarginal(
			endpointPairs, requireComplete);
	}
	...
	ZhangCapturedFactorEvidenceWindow factorEvidenceWindow() const
	{
		ZhangCapturedFactorEvidenceWindow result;
		const ZhangCapturedFactorEvent* firstMeasurement = nullptr;
		for (const auto& event : events)
		{
			if (event.kind != ZhangCapturedFactorKind::MEASUREMENT) continue;
			if (!firstMeasurement) firstMeasurement = &event;
			result.measurementSequences.push_back(event.sequence);
			result.measurementRows += event.design.rows();
		}
		if (!firstMeasurement || result.measurementSequences.empty())
		{
			result.failureReason = "NO_CAPTURED_MEASUREMENT_FACTORS";
			return result;
		}
		if (!std::is_sorted(result.measurementSequences.begin(),
				result.measurementSequences.end()) ||
			std::adjacent_find(result.measurementSequences.begin(),
				result.measurementSequences.end()) !=
				result.measurementSequences.end())
		{
			result.failureReason = "MEASUREMENT_FACTOR_SEQUENCE_INVALID";
			return result;
		}

		// Deterministic FNV-1a fingerprint of the anchor coordinate.  Storing the
		// complete (often very large) key list in every certificate is unnecessary;
		// the fingerprint is only a chronology namespace, while the exact factor
		// IDs remain the sequence vector below.
		...
		auto mixInteger = [&](std::uint64_t value)
		{
			for (int byte = 0; byte < 8; byte++)
				mixByte(static_cast<unsigned char>((value >> (8 * byte)) & 0xff));
		};
		for (const auto& key : anchorKeys)
		{
			mixInteger(static_cast<std::uint64_t>(key.type));
			mixInteger(static_cast<std::uint64_t>(key.number));
			mixInteger(static_cast<std::uint64_t>(key.satellite.sys));
			mixInteger(static_cast<std::uint64_t>(key.satellite.prn));
			mixByte(0xff);
			for (const char value : key.receiver)
				mixByte(static_cast<unsigned char>(value));
			mixByte(0xfe);
		}
		std::ostringstream identity;
		identity << "RAW_MEASUREMENT_PREFIX|anchor_time="
			<< static_cast<long>(firstMeasurement->time.bigTime)
			<< "|anchor_keys_fnv1a=" << std::hex << fingerprint
			<< "|first_label=" << firstMeasurement->label;
		result.identity = identity.str();
		result.firstSequence = result.measurementSequences.front();
		result.lastSequence = result.measurementSequences.back();
		result.valid = !result.identity.empty() && result.measurementRows > 0;
		result.failureReason = result.valid ? "NONE" :
			"CAPTURED_MEASUREMENT_FACTOR_WINDOW_INVALID";
		return result;
	}
	...
	ZhangFactorCaptureSummary summary() const
	{
		...
		std::set<std::string> measurementFactorIds;
		std::uint64_t chronologyHash = 1469598103934665603ULL;
		for (const auto& event : events)
		{
			zhangFactorIdAppendString(chronologyHash, event.factorId);
			if (event.kind == ZhangCapturedFactorKind::MEASUREMENT
			 && !measurementFactorIds.insert(event.factorId).second)
			{
				result.duplicateMeasurementFactorIds++;
			}
		}
		result.uniqueMeasurementFactorIds = measurementFactorIds.size();
		std::ostringstream chronologyIdentity;
		chronologyIdentity << std::hex << std::setw(16) << std::setfill('0')
			<< chronologyHash;
		result.factorChronologyHash = chronologyIdentity.str();
		...
		result.maximumCommitMeanRelativeError =
			maximumCommitMeanRelativeError;
		result.maximumCommitCovarianceRelativeError =
			maximumCommitCovarianceRelativeError;
		result.lastCommitSequence = lastCommitSequence;
	...
	ZhangFactorCaptureRuntimeReplay checkpointReplay() const
	{
		...
		result.maximumCommitCovarianceRelativeError =
			maximumCommitCovarianceRelativeError;
		result.lastCommitSequence = lastCommitSequence;
		result.commitSequenceAnchored = commitSequenceAnchored;
		result.maximumTargetMeanRelativeError = maximumTargetMeanRelativeError;
		result.maximumTargetVarianceRelativeError =
			maximumTargetVarianceRelativeError;
		result.maximumRawSquareRootMeanRelativeError =
		...
		std::string* failureReason = nullptr)
	{
		...
		if (snapshot.events.empty())
		{
			...
			candidate.maximumCommitMeanRelativeError =
				snapshot.maximumCommitMeanRelativeError;
			candidate.maximumCommitCovarianceRelativeError =
				snapshot.maximumCommitCovarianceRelativeError;
			candidate.lastCommitSequence = snapshot.lastCommitSequence;
			candidate.commitSequenceAnchored =
				snapshot.commitSequenceAnchored;
		...
		for (std::size_t index = 0; index < snapshot.events.size(); index++)
		{
			...
			if (event.kind == ZhangCapturedFactorKind::MEASUREMENT)
			{
				...
				if (!candidate.recordMeasurement(
						...
						posteriorCovariance, event.beforeCommitSequence,
						event.afterCommitSequence))
				{
			...
			else if (event.kind == ZhangCapturedFactorKind::STATE_TRANSITION)
			{
				...
				const VectorXd affineOffset = event.rightHandSide.size() == 0
					? VectorXd::Zero(transition.rows()) : event.rightHandSide;
				const VectorXd committedMean = transition * mean + affineOffset;
				MatrixXd committedCovariance = transition * covariance
					* transition.transpose() + process;
				committedCovariance = 0.5
					* (committedCovariance + committedCovariance.transpose());
				if (!candidate.recordTransition(
						event.time, event.sourceKeys, event.destinationKeys,
						event.design, process, affineOffset, event.label,
						committedMean, committedCovariance,
						event.beforeCommitSequence,
						event.afterCommitSequence))
				{
				...
				candidate.events.back() = event;
				mean = committedMean;
				covariance = committedCovariance;
			}
			else
			{
				const MatrixXd transform = MatrixXd(event.design);
				const VectorXd affineOffset = event.rightHandSide.size() == 0
					? VectorXd::Zero(transform.rows()) : event.rightHandSide;
				const VectorXd committedMean = transform * mean + affineOffset;
				MatrixXd committedCovariance = transform * covariance
					* transform.transpose();
				committedCovariance = 0.5
					* (committedCovariance + committedCovariance.transpose());
				if (!candidate.recordCoordinateTransform(
						...
						event.preserveUnrepresentablePersistentTargets,
						affineOffset, committedMean, committedCovariance,
						event.beforeCommitSequence,
						event.afterCommitSequence))
				{
		...
		candidate.maximumCommitCovarianceRelativeError =
			snapshot.maximumCommitCovarianceRelativeError;
		candidate.lastCommitSequence = snapshot.lastCommitSequence;
		candidate.commitSequenceAnchored = snapshot.commitSequenceAnchored;
		candidate.maximumTargetMeanRelativeError =
			snapshot.maximumTargetMeanRelativeError;
		candidate.maximumTargetVarianceRelativeError =
			snapshot.maximumTargetVarianceRelativeError;
		...
		double processNoiseScale = 1) const
	{
		...
		for (const auto& event : events)
		{
			...
			else if (event.kind == ZhangCapturedFactorKind::STATE_TRANSITION)
			{
				...
				if (!replay.advance(
						design, processCovariance, event.rightHandSide))
				{
					rejected.failureReason = replay.lastFailureReason();
					return rejected;
				}
			}
			else if (!replay.applyExactCoordinateTransform(
					design, event.rightHandSide))
			{
				rejected.failureReason = replay.lastFailureReason();
				return rejected;
			}
...
private:
		...
		bool failIfUnrepresentable)
	{
		for (auto& [identity, history] : histories)
		{
			...
			history.keys = destination;
			if (affineOffset != nullptr)
			{
				history.offset -= destinationRow.dot(*affineOffset);
			}
