 ...
 */
bool KFState::manualStateTransition(
    Trace& trace,
    GTime newTime,
    MatrixXd& F,
    MatrixXd& Q0,
    const string& label)
{
    const bool stagedTransaction = activeMeasurementTransaction != nullptr;
    KFState_ operationEntry;
    if (!stagedTransaction)
    {
        operationEntry = static_cast<const KFState_&>(*this);
    }
    const GTime destinationTime = newTime == GTime::noTime() ? time : newTime;
    const VectorXd sourceMean = x;
    const int sourceCovarianceRows = P.rows();
    const int sourceCovarianceCols = P.cols();
    const auto sourceIndexMap = kfIndexMap;
    ...
    {
        if (activeMeasurementTransaction)
        {
            activeMeasurementTransaction->primaryError =
                "INVALID_MANUAL_STATE_TRANSITION_MOMENTS:" + label;
        }
        return false;
    }
    ...
    {
        destinationMean = sourceMean;
        destinationCovariance = P;
    }

    const std::uint64_t beforeCommitSequence = activeMeasurementTransaction
        ? activeMeasurementTransaction->provisionalCommitSequence
        : factorCommitSequence;
    const std::uint64_t afterCommitSequence = beforeCommitSequence + 1;
    ...
    if (activeMeasurementTransaction)
    {
        ...
        if (!simulate_filter_only)
        {
            dx = destinationMean - sourceMean;
            x  = destinationMean;
            P  = destinationCovariance;
        }
        initFilterEpoch(trace);

        if (kfIndexMap != sourceIndexMap
            || x.size() != sourceMean.size()
            || P.rows() != sourceCovarianceRows
            || P.cols() != sourceCovarianceCols)
        {
            activeMeasurementTransaction->primaryError =
                "INTERNAL_TRANSITION_CHANGED_KEY_TOPOLOGY:" + label;
            return false;
        }

        KFStagedFactor staged;
        staged.kind = KFStagedFactorKind::STATE_TRANSITION;
        staged.time = destinationTime;
        staged.label = label;
        staged.adaptiveQcTransition =
            label.find("KF_POSTFIT_STATE_RELAXATION/") == 0;
        staged.sourceIndexMap = sourceIndexMap;
        staged.destinationIndexMap = kfIndexMap;
        staged.transition = F.sparseView();
        staged.processCovariance = Q0.sparseView();
        staged.beforeCommitSequence = beforeCommitSequence;
        staged.afterCommitSequence = afterCommitSequence;
        activeMeasurementTransaction->stagedFactors.push_back(std::move(staged));
        activeMeasurementTransaction->provisionalCommitSequence = afterCommitSequence;

        string stateKey = label;
        if (auto separator = stateKey.rfind('/'); separator != string::npos)
        {
            stateKey = stateKey.substr(separator + 1);
        }
        trace << "\nZHANG_KF_MEASUREMENT_TRANSACTION"
              << " time=" << destinationTime.to_string(0)
              << " boundary=B11"
              << " status=ADAPTIVE_QC_TRANSITION"
              << " label=" << label
              << " state_key=" << stateKey
              << " before_commit_sequence=" << beforeCommitSequence
              << " after_commit_sequence=" << afterCommitSequence
              << " provisional_commit_sequence="
              << activeMeasurementTransaction->provisionalCommitSequence
              << " staged_factor_count="
              << activeMeasurementTransaction->stagedFactors.size()
              << " feedback=0";
        return true;
    }
    ...
    {
        lastFactorTransactionFailureReason =
            "MANUAL_TRANSITION_CALLBACK_EXCEPTION:" + string(exception.what());
        factorAccepted = false;
    }
    catch (...)
    {
        lastFactorTransactionFailureReason =
            "MANUAL_TRANSITION_CALLBACK_EXCEPTION:UNKNOWN";
        factorAccepted = false;
    }
    if (!factorAccepted)
    {
        const string failure = lastFactorTransactionFailureReason;
        static_cast<KFState_&>(*this) = operationEntry;
        lastFactorTransactionFailureReason = failure.empty()
            ? "MANUAL_TRANSITION_FACTOR_REJECTED:" + label
            : failure;
        return false;
    }

    // RTS output is emitted only after the factor consumer accepts the commit.
    ...
    {
        dx = destinationMean - sourceMean;
        x  = std::move(destinationMean);
        P  = std::move(destinationCovariance);
    }
    factorCommitSequence = afterCommitSequence;
...
)
{
    ...
    const std::uint64_t beforeCommitSequence = factorCommitSequence;
    const std::uint64_t afterCommitSequence  = beforeCommitSequence + 1;
    bool factorAccepted = true;
    try
    {
    ...
    catch (const std::exception& exception)
    {
        factorAccepted = false;
        lastFactorTransactionFailureReason =
            "STATE_TRANSFORM_CALLBACK_EXCEPTION:" + string(exception.what());
    }
    catch (...)
    {
        factorAccepted = false;
        lastFactorTransactionFailureReason =
            "STATE_TRANSFORM_CALLBACK_EXCEPTION:UNKNOWN";
    }
    if (!factorAccepted)
    {
        const string failure = lastFactorTransactionFailureReason;
        static_cast<KFState_&>(*this) = operationEntry;
        lastFactorTransactionFailureReason = failure.empty()
            ? "STATE_TRANSFORM_FACTOR_REJECTED:" + label
            : failure;
        return false;
    }
...
)
{
    ...
    if (newStateCount == 0)
    {
        ...
        static_cast<KFState_&>(*this) = operationEntry;
        lastFactorTransactionFailureReason = "STATE_TRANSITION_HAS_NO_STATES";
        return false;
    }
...
#pragma omp parallel for schedule(dynamic) if (parallelChunks)
        ...
        destinationCovariance = std::move(Pnew);
    }
    else
    {
        destinationCovariance = (F * P * F.transpose() + Q0).eval();
    }
    ...
    catch (const std::exception& exception)
    {
        factorAccepted = false;
        lastFactorTransactionFailureReason =
            "STATE_TRANSITION_CALLBACK_EXCEPTION:" + string(exception.what());
    }
    catch (...)
    {
        factorAccepted = false;
        lastFactorTransactionFailureReason =
            "STATE_TRANSITION_CALLBACK_EXCEPTION:UNKNOWN";
    }
    if (!factorAccepted)
    {
        const string failure = lastFactorTransactionFailureReason;
        static_cast<KFState_&>(*this) = operationEntry;
        lastFactorTransactionFailureReason = failure.empty()
            ? "STATE_TRANSITION_FACTOR_REJECTED"
            : failure;
        return false;
    }

    // External outputs become visible only after the factor consumer accepts.
    if (rts_basename.empty() == false)
    {
        TransitionMatrixObject transitionMatrixObject = F;
        spitFilterToFile(
...
)
{
    ...
    // BLAS DGEMV requires lda >= max(1,m).  A measurement-only chunk with no
    // state columns previously reached the state update with numX == lda == 0,
    // producing "parameter number 6 illegal" and leaving an undefined update.
    // Reject malformed/empty chunks before forming any raw block pointer.
    const bool validBlock = validKalmanFilterBlasBlock(
        begX, numX, begH, numH, H.rows(), H.cols(), P.rows(), P.cols(),
        R.rows(), R.cols(), V.rows());
    if (!validBlock)
    {
        trace << "\nINVALID KALMAN FILTER BLOCK"
              << " begX=" << begX << " numX=" << numX
              << " begH=" << begH << " numH=" << numH
              << " h_rows=" << H.rows() << " h_cols=" << H.cols()
              << " action=REJECT_BEFORE_BLAS";
        if (resetOnFailure)
        {
            xp = x;
            Pp = P;
...
{
    if (kfMeas.H.rows() <= 0 || kfMeas.H.cols() <= 0 ||
        kfMeas.V.rows() != kfMeas.H.rows() ||
        kfMeas.R.rows() != kfMeas.H.rows() ||
        kfMeas.R.cols() != kfMeas.H.rows())
    {
        trace << "\nINVALID LEAST SQUARE DIMENSIONS"
              << " h_rows=" << kfMeas.H.rows()
              << " h_cols=" << kfMeas.H.cols()
              << " v_rows=" << kfMeas.V.rows()
              << " r_rows=" << kfMeas.R.rows()
              << " r_cols=" << kfMeas.R.cols()
              << " action=REJECT_BEFORE_BLAS";
        return false;
    }
    ...
    // Materialise the weighted RHS before the state-space multiply.  Besides
    // avoiding a backend-dependent chained GEMV evaluation, the dimensions are
    // now explicit and covered by the guards above.
    const VectorXd weightedRhs = H_W * V;
    if (weightedRhs.rows() != numX || !weightedRhs.allFinite())
    {
        trace << "\nINVALID LEAST SQUARE WEIGHTED RHS";
        return false;
    }
    xp.noalias() = Pp * weightedRhs;
 ...
 */
KFFilterResult KFState::filterKalman(
    Trace&                    trace,       ///< Trace file for output
    ...
    map<string, FilterChunk>* filterChunkMap_ptr,
    KFState*                  finalMeasurementPrior_ptr
)
{
    ...
    {
        lastPppTransactionCommitted = false;
        lastPppQcConverged = false;
    }
    if (authoritativePppTransaction && activeMeasurementTransaction)
    {
        lastFactorTransactionFailureReason = "NESTED_MEASUREMENT_TRANSACTION";
        trace << "\nZHANG_KF_MEASUREMENT_TRANSACTION"
              << " boundary=B13 status=ROLLED_BACK"
              << " reason=" << lastFactorTransactionFailureReason;
        return KFFilterResult::FACTOR_TRANSACTION_REJECTED;
    }
    ...
    transaction.trace_ptr = &trace;
    transaction.entryCommitSequence = factorCommitSequence;
    transaction.provisionalCommitSequence = factorCommitSequence;
    transaction.transactionId = transaction.time.to_string(0) + "|" + suffix + "|" +
        std::to_string(transaction.entryCommitSequence + 1);
    if (authoritativePppTransaction)
    {
        transaction.entryIndexMap = std::move(operationEntry.kfIndexMap);
        transaction.entryMean = std::move(operationEntry.x);
        transaction.entryCovariance = std::move(operationEntry.P);
        activeMeasurementTransaction = &transaction;
    }

    struct ActiveMeasurementTransactionGuard
    {
        KFState& state;
        KFMeasurementTransaction* transaction_ptr;

        ~ActiveMeasurementTransactionGuard()
        {
            if (state.activeMeasurementTransaction == transaction_ptr)
            {
                state.activeMeasurementTransaction = nullptr;
            }
        }
    } transactionGuard{*this, authoritativePppTransaction ? &transaction : nullptr};

    auto restoreTransactionEntry = [&](KFFilterResult result, const string& reason)
    {
        // Every caller returns immediately after rollback, so transfer the
        // saved packet back instead of deep-copying the multi-gigabyte dense
        // covariance and measurement matrices a second time.  The previous
        // copy path could throw std::bad_alloc while handling the original
        // numerical failure, hiding the real rejection reason.
        static_cast<KFState_&>(*this) = std::move(operationEntry);
        if (authoritativePppTransaction)
        {
            kfIndexMap = std::move(transaction.entryIndexMap);
            x = std::move(transaction.entryMean);
            P = std::move(transaction.entryCovariance);
        }
        activeMeasurementTransaction = nullptr;
        kfMeas = std::move(measurementEntry);
        lastFactorTransactionFailureReason = reason;
        if (authoritativePppTransaction)
        {
            lastPppTransactionCommitted = false;
            lastPppQcConverged = false;
    ...
    {
        resolveKalmanFilterChunkExtents(fc, x.rows(), kfMeas.H.rows());
    }

    if (kfMeas.H.rows() == 0)
    {
        return restoreTransactionEntry(
            KFFilterResult::NO_MEASUREMENTS,
            "NO_MEASUREMENTS");
    }
    ...
    for (auto& [id, fc] : filterChunkMap)
    {
        ...
        for (int i = 0; i < prefitOpts.max_iterations; i++)
        {
            ...
            if (!transaction.primaryError.empty())
            {
                trace << stringBuffer.str();
                if (fc.trace_ptr)
                    *fc.trace_ptr << stringBuffer.str();
                return restoreTransactionEntry(
                    KFFilterResult::FACTOR_TRANSACTION_REJECTED,
                    transaction.primaryError);
            }
    ...
    for (auto& [id, fc] : filterChunkMap)
    {
        if (!activeKalmanFilterStateChunk(fc.numX, fc.numH))
        {
            if (fc.numH > 0 && fc.numX <= 0)
            {
                trace << "\nZHANG_KF_MEASUREMENT_ONLY_CHUNK"
                      << " id=" << id
                      << " begX=" << fc.begX
                      << " numX=" << fc.numX
                      << " begH=" << fc.begH
    ...
    vector<std::size_t> chunkSolveGenerations(
        activeChunks.size(), std::numeric_limits<std::size_t>::max());

    if (activeChunks.empty() || postfitOpts.max_iterations <= 0)
    {
        return restoreTransactionEntry(
            KFFilterResult::NUMERICAL_FAILURE,
...
#pragma omp parallel for schedule(dynamic) if (parallelChunks)
    ...
    for (int c = 0; c < activeChunks.size(); c++)
    {
        ...
        for (int i = 0; i < postfitOpts.max_iterations; i++)
        {
            bool pass = false;
            if (i == 0
                && chunkResults[c].computed
                && chunkResults[c].modelGeneration == transaction.modelGeneration)
            {
            ...
            if (pass == false)
            {
                ...
                return restoreTransactionEntry(
                    KFFilterResult::NUMERICAL_FAILURE,
                    "KALMAN_CHUNK_SOLVE_FAILED:" + fc.id);
            }

            chunkSolveGenerations[c] = transaction.modelGeneration;
    ...
    {
        if (chunkSolveGenerations[c] != transaction.modelGeneration)
        {
            modelSolveMismatch = true;
            break;
        }
    }
    ...
    if (transaction.modelGeneration > 0 || modelSolveMismatch)
    {
        trace << "\nZHANG_KF_MEASUREMENT_TRANSACTION"
              << " time=" << transaction.time.to_string(0)
              << " boundary=B12 status=TERMINAL_RECONCILIATION_BEGIN"
              << " transaction_id=" << transaction.transactionId
              << " model_generation=" << transaction.modelGeneration
              << " solve_generation=" << transaction.solveGeneration
              << " reject_callbacks_enabled=0";

        xp = x;
        Pp = P;
        dx = VectorXd::Zero(x.rows());
        for (std::size_t c = 0; c < activeChunks.size(); c++)
        {
            auto& fc = *activeChunks[c];
            MatrixXd Qinv = MatrixXd::Identity(fc.numH, fc.numH);
            MatrixXd QinvH = MatrixXd::Ones(fc.numH, fc.numX);
            const bool pass = kFilter(
                trace,
                kfMeas,
                xp,
                Pp,
            ...
            if (!pass)
            {
                return restoreTransactionEntry(
                    KFFilterResult::NUMERICAL_FAILURE,
                    "TERMINAL_RECONCILIATION_FAILED:" + fc.id);
            }
            chunkSolveGenerations[c] = transaction.modelGeneration;
        }
        ...
        trace << "\nZHANG_KF_MEASUREMENT_TRANSACTION"
              << " time=" << transaction.time.to_string(0)
              << " boundary=B12 status=TERMINAL_RECONCILIATION_COMPLETED"
              << " transaction_id=" << transaction.transactionId
              << " model_generation=" << transaction.modelGeneration
              << " solve_generation=" << transaction.solveGeneration
              << " reject_callbacks_enabled=0";
    ...
    const std::uint64_t beforeCommitSequence = authoritativePppTransaction
        ...
        : factorCommitSequence;
    const std::uint64_t afterCommitSequence  = beforeCommitSequence + 1;
    const string priorMomentHash = kfMomentHash(x, P);
    const string posteriorMomentHash = kfMomentHash(xp, Pp);

    if (authoritativePppTransaction)
    {
        KFStagedFactor staged;
        staged.kind = KFStagedFactorKind::MEASUREMENT;
        staged.time = transaction.time;
        staged.label = suffix;
        staged.measurement_ptr = &kfMeas;
        staged.destinationMean_ptr = &xp;
        staged.destinationCovariance_ptr = &Pp;
        staged.beforeCommitSequence = beforeCommitSequence;
        staged.afterCommitSequence = afterCommitSequence;
        transaction.stagedFactors.push_back(std::move(staged));
        transaction.provisionalCommitSequence = afterCommitSequence;
    }

    if (finalMeasurementPrior_ptr)
    {
        if (finalMeasurementPrior_ptr == this)
        {
            return restoreTransactionEntry(
                KFFilterResult::FACTOR_TRANSACTION_REJECTED,
                "FINAL_MEASUREMENT_PRIOR_ALIASES_FILTER");
        }
    ...
    try
    {
        if (authoritativePppTransaction && authoritativeFactorTransactionCallback)
        {
            factorAccepted = authoritativeFactorTransactionCallback(
                *this, transaction, factorFailureReason);
        }
        else if (acceptedMeasurementFactorCallback)
        {
            factorAccepted = acceptedMeasurementFactorCallback(
                *this,
                kfMeas,
                suffix,
                xp,
    ...
    {
        factorAccepted = false;
        factorFailureReason = "MEASUREMENT_FACTOR_CALLBACK_EXCEPTION:" +
            string(exception.what());
    }
    catch (...)
    {
        factorAccepted = false;
        factorFailureReason = "MEASUREMENT_FACTOR_CALLBACK_EXCEPTION:UNKNOWN";
    }
    if (!factorAccepted)
    {
        if (factorFailureReason.empty())
        {
            factorFailureReason = authoritativePppTransaction &&
                authoritativeFactorTransactionCallback
                ? "AUTHORITATIVE_FACTOR_TRANSACTION_REJECTED"
                : "MEASUREMENT_FACTOR_REJECTED";
        }
        return restoreTransactionEntry(
            KFFilterResult::FACTOR_TRANSACTION_REJECTED,
            factorFailureReason);
    ...
    // Only now are shallow-pointer side effects from reject callbacks allowed
    // to become visible.  They are intentionally absent from every rollback
    // path above.
    for (auto& commitSideEffect : transaction.commitSideEffects)
    {
        commitSideEffect();
    }

    // No durable RTS record is emitted until the complete estimator/sidecar
    ...
    if (authoritativePppTransaction)
    {
        lastPppTransactionCommitted = true;
        lastPppQcConverged = transaction.qcConverged;
    }

    std::size_t transitionFactorCount = 0;
    for (const auto& staged : transaction.stagedFactors)
    {
        if (staged.kind == KFStagedFactorKind::STATE_TRANSITION)
        {
            transitionFactorCount++;
        }
    }
    const string entryMomentHash = authoritativePppTransaction
        ? kfMomentHash(transaction.entryMean, transaction.entryCovariance)
        : "NOT_APPLICABLE";

    trace << "\nZHANG_KF_MEASUREMENT_TRANSACTION"
          << " time=" << transaction.time.to_string(0)
          << " boundary=B13 status=COMMITTED"
          << " transaction_id=" << transaction.transactionId
          << " entry_commit_sequence=" << transaction.entryCommitSequence
          << " final_commit_sequence=" << factorCommitSequence
          << " model_generation=" << transaction.modelGeneration
          << " solve_generation=" << transaction.solveGeneration
          << " qc_converged=" << transaction.qcConverged
          << " max_iterations_reached=" << transaction.maxIterationsReached
          << " terminal_reconciliation_performed="
          << transaction.terminalReconciliationPerformed
          << " staged_transition_count=" << transitionFactorCount
          << " staged_factor_count=" << transaction.stagedFactors.size()
          << " entry_moment_hash=" << entryMomentHash
          << " b13_prior_hash=" << priorMomentHash
          << " posterior_moment_hash=" << posteriorMomentHash
          << " feedback=0";
    ...
    if (authoritativePppTransaction)
    {
        trace << "\nZHANG_KF_MEASUREMENT_TRANSACTION"
              << " time=" << transaction.time.to_string(0)
              << " boundary=B16 status=AUTHORITATIVE_COMMIT"
              << " transaction_id=" << transaction.transactionId
              << " model_generation=" << transaction.modelGeneration
              << " solve_generation=" << transaction.solveGeneration
              << " final_commit_sequence=" << factorCommitSequence
...
)
{
    ...
    // Build the initialised state atomically.  A negative diagonal is an
    // internal sentinel for a state which does not yet have a stochastic
    // prior; it must never be exposed to factor consumers as though it were
    // an ordinary covariance update.  In particular, reusing the same KFKey
    // for a fresh ambiguity arc must retire the old random variable rather
    // than transport its historical information into the new arc.
    VectorXd initialisedX  = x;
    VectorXd initialisedDx = dx;
    MatrixXd initialisedP  = P;
    vector<int> initialisedStateIndicies;
    ...
    for (int i = 0; i < usedStateCount; i++)
    {
        ...
        initialisedDx(stateRowIndex) = newStateVal;

        if (innovReady)
        {
            initialisedX(stateRowIndex) += newStateVal;
        }
        else
        {
            initialisedX(stateRowIndex) = newStateVal;
        ...
        initialisedP(stateRowIndex, stateRowIndex) = newStateCov;
        initialisedStateIndicies.push_back(stateRowIndex);
        ...
        if (initCovars)
        {
            for (int j = 0; j < i; j++)
            {
                ...
                initialisedP(stateRowIndex, stateColIndex) = newStateCov;
                initialisedP(stateColIndex, stateRowIndex) = newStateCov;
            }
        }
    }

    if (!initialisedStateIndicies.empty())
    {
        // The LSQ operation replaces each uninitialised random variable.  Its
        // authoritative transaction is therefore a reset transition, not an
        // identity transition and not another measurement factor.  Zeroing
        // the affected rows prevents a previous arc with the same semantic
        // key from leaking into the fresh state.  Q then contains exactly the
        // covariance of the newly initialised variables (and is diagonal for
        // the production PPP call where initCovars=false).
        SparseMatrix<double> initialisationTransition;
        MatrixXd initialisationProcessCovariance;
        if (!makeKFStateInitialisationTransition(
                P, initialisedP, initialisedStateIndicies,
                initialisationTransition, initialisationProcessCovariance))
        {
            trace << "\nINVALID LEAST SQUARE STATE INITIALISATION TRANSACTION";
            return false;
        }
        ...
        if (stateTransitionFactorCallback)
        {
            try
            {
                factorAccepted = stateTransitionFactorCallback(
                    *this,
                    ...
                    kfIndexMap,
                    kfIndexMap,
                    initialisationTransition,
                    initialisationProcessCovariance,
                    "KF_LEAST_SQUARE_STATE_INITIALISATION" + suffix,
                    initialisedX,
                    initialisedP,
                    beforeCommitSequence,
                    afterCommitSequence);
            ...
            {
                lastFactorTransactionFailureReason =
                    "LSQ_INITIALISATION_CALLBACK_EXCEPTION:" + string(exception.what());
                factorAccepted = false;
            }
            catch (...)
            {
                lastFactorTransactionFailureReason =
                    "LSQ_INITIALISATION_CALLBACK_EXCEPTION:UNKNOWN";
                factorAccepted = false;
            }
        }
        if (!factorAccepted)
        {
            if (lastFactorTransactionFailureReason.empty())
            {
                lastFactorTransactionFailureReason =
                    "LSQ_INITIALISATION_FACTOR_REJECTED" + suffix;
            }
            trace << "\n" << lastFactorTransactionFailureReason;
            return false;
        }

        x  = std::move(initialisedX);
        dx = std::move(initialisedDx);
        P  = std::move(initialisedP);
        factorCommitSequence = afterCommitSequence;
    }
