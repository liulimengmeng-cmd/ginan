...
)
{
        ...
        // 		processing_options
        {
            ...
            // 			gnss_general
            {
...

                {
					...
					tryGetFromYaml(
						zhangPppAr.tree_slip_shadow_replay,
						zhang_pppar,
						{"@ tree_slip_shadow_replay"},
						"Evaluate pivot-before-retire on a KFState copy without authoritative feedback"
					);
					tryGetFromYaml(
						zhangPppAr.tree_slip_shadow_max_events,
						zhang_pppar,
						{"@ tree_slip_shadow_max_events"},
						"Maximum tree-slip events retained by the shadow replay audit"
					);
					tryGetFromYaml(
						zhangPppAr.tree_slip_pivot_before_retire,
						zhang_pppar,
						{"@ tree_slip_pivot_before_retire"},
						"Authoritatively pivot the surviving represented state graph before retiring a slipped tree arc"
					);
                    tryGetFromYaml(
					...
					tryGetFromYaml(
						zhangPppAr.fixed_lag_deterministic_replay_stride,
						zhang_pppar,
						{"@ fixed_lag_deterministic_replay_stride"},
						"Run the diagnostic-only deterministic A/B raw-factor replay every N accepted measurement blocks"
					);
					tryGetFromYaml(
						zhangPppAr.fixed_lag_product_relation_completion,
						zhang_pppar,
						{"@ fixed_lag_product_relation_completion"},
						"Use the complete fixed-lag joint product-target marginal for private integer candidate proposal; current-posterior gates remain authoritative"
					);
					tryGetFromYaml(
                    ...
                    tryGetFromYaml(
                        zhangPppAr.product_joint_dual_frequency_ils,
                        zhang_pppar,
                        {"@ product_joint_dual_frequency_ils"},
                        "Use one current-posterior joint [WL,L1] ILS/PAR solve and bypass the staged product WL then conditional-L1 search"
                    );
                    tryGetFromYaml(
    ...
    if (zhangPppAr.output_products || zhangPppAr.user_adapter)
    {
		...
		if (zhangPppAr.fixed_lag_deterministic_replay_stride < 1)
		{
			BOOST_LOG_TRIVIAL(error)
				<< "zhang_pppar fixed_lag_deterministic_replay_stride must be positive";
			valid = false;
		}
        ...
        zhangPppAr.product_relation_l1_par_shadow =
            productRelationSolverMode == "SHADOW";
        if (zhangPppAr.product_joint_dual_frequency_ils &&
            (!boost::iequals(
        ...
        {
            BOOST_LOG_TRIVIAL(error)
                << "zhang_pppar joint dual-frequency product ILS requires "
		...
		if (zhangPppAr.tree_slip_shadow_max_events < 1)
		{
			BOOST_LOG_TRIVIAL(error)
				<< "zhang_pppar tree_slip_shadow_max_events must be positive";
			valid = false;
		}
		...
		if (zhangPppAr.fixed_lag_product_relation_completion &&
			(productMode != "HOU_OSB_LIKE" ||
			 integerStrategy != "HYBRID_PRODUCT_WL_L1" ||
			 besdCapturePolicy != "ALL" ||
			 !zhangPppAr.temporal_product_transition_shadow ||
		...
		{
			BOOST_LOG_TRIVIAL(error)
				<< "fixed_lag_product_relation_completion requires "
				   "product_mode=HOU_OSB_LIKE, "
				   "integer_strategy=HYBRID_PRODUCT_WL_L1, "
				   "besd_capture_policy=ALL, "
				   "temporal_product_transition_shadow=true, and "
				   "fixed_lag_factor_capture_shadow=true, and "
				   "transactional_integer_fixing=true";
			valid = false;
		}
