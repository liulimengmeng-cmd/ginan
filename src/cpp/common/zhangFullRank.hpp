...
/** Plan an exact pivot before retiring one or more slipped physical arcs.
 *
 * The replacement tree is selected only from the pre-event represented graph
 * after removing the slipped arcs.  A tree obtained from the instantaneous
 * observation graph is not sufficient: a newly observed post-slip arc has no
 * old-state expression and therefore cannot authorise an exact S-transform.
 * The plan succeeds only when every pre-event receiver and satellite remains
 * connected to the old root through surviving represented arcs. */
struct ZhangPivotBeforeRetirePlan
{
    bool required = false;
    bool connected = false;
    std::set<ZhangGraphEdge> slippedTreeEdges;
    std::set<ZhangGraphEdge> survivingRepresentedEdges;
    std::set<ZhangGraphEdge> replacementEdges;
    ZhangGraphBasis replacementBasis;
    std::string failureReason = "NOT_EVALUATED";
};

/** Intersect the represented edge sets of every observable participating in a
 * joint product.  This prevents a replacement edge that exists only in an
 * L1-or-L2 union graph from being selected for a dual-frequency S-transform. */
inline std::set<ZhangGraphEdge> zhangJointObservableEdgeIntersection(
    const std::vector<std::set<ZhangGraphEdge>>& perObservableEdges)
{
    if (perObservableEdges.empty())
    {
        return {};
    }
    std::set<ZhangGraphEdge> result = perObservableEdges.front();
    for (std::size_t index = 1; index < perObservableEdges.size(); index++)
    {
        std::set<ZhangGraphEdge> intersection;
        std::set_intersection(
            result.begin(), result.end(),
            perObservableEdges[index].begin(), perObservableEdges[index].end(),
...
inline ZhangPivotBeforeRetirePlan zhangPlanPivotBeforeRetire(
    const ZhangGraphBasis&          oldBasis,
    const std::set<ZhangGraphEdge>& representedEdges,
    const std::set<ZhangGraphEdge>& slippedEdges)
{
    ZhangPivotBeforeRetirePlan result;
    std::set_intersection(
        oldBasis.treeEdges.begin(), oldBasis.treeEdges.end(),
        slippedEdges.begin(), slippedEdges.end(),
        std::inserter(result.slippedTreeEdges,
                      result.slippedTreeEdges.begin()));
    result.required = !result.slippedTreeEdges.empty();
    if (!result.required)
    {
        result.failureReason = "NO_SLIPPED_TREE_EDGE";
        return result;
    }

    std::set_difference(
        representedEdges.begin(), representedEdges.end(),
        slippedEdges.begin(), slippedEdges.end(),
        std::inserter(result.survivingRepresentedEdges,
                      result.survivingRepresentedEdges.begin()));
    const auto rootEdges = zhangRootComponentEdges(
        result.survivingRepresentedEdges, oldBasis.rootReceiver);
    std::set<ZhangGraphEdge> survivingPreferred;
    std::set_difference(
        oldBasis.treeEdges.begin(), oldBasis.treeEdges.end(),
        slippedEdges.begin(), slippedEdges.end(),
        std::inserter(survivingPreferred, survivingPreferred.begin()));
    result.replacementBasis = zhangBuildSpanningTree(
        rootEdges,
        oldBasis.rootReceiver,
        survivingPreferred,
        {},
        result.survivingRepresentedEdges);
    if (!result.replacementBasis.connected ||
        result.replacementBasis.receivers != oldBasis.receivers ||
        result.replacementBasis.satellites != oldBasis.satellites)
    {
        result.failureReason = "SURVIVING_REPRESENTED_GRAPH_DISCONNECTED";
        return result;
    }
    std::set_difference(
        result.replacementBasis.treeEdges.begin(),
        result.replacementBasis.treeEdges.end(),
        oldBasis.treeEdges.begin(), oldBasis.treeEdges.end(),
        std::inserter(result.replacementEdges,
                      result.replacementEdges.begin()));
