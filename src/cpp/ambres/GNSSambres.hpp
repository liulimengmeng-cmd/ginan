...
struct GinAR_mtx
{
    // Dependencies of the ACTUALLY APPLIED posterior / returned integer rows.
    ZhangDecisionProofs decisionProofs;
    // Proofs for returned Ztrs rows, distinct from the whole conditioned posterior.
    std::vector<ZhangDecisionProofs> rowDecisionProofs;
