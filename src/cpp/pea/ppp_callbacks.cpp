...
bool deweightMeas(RejectCallbackDetails rejectDetails)
{
    ...
    if (obs_ptr)
    {
        ...
        if (kfState.activeMeasurementTransaction)
        {
            kfState.activeMeasurementTransaction->commitSideEffects.push_back(
                [obs_ptr]() { obs_ptr->excludeOutlier = true; });
        }
        else
        {
    ...
    if (otherNoiseMatrix_ptr)
    {
        ...
        {
            kfState.activeMeasurementTransaction->commitSideEffects.push_back(
                [otherNoiseMatrix_ptr, otherIndex, deweightFactor]()
                {
                    otherNoiseMatrix_ptr->row(otherIndex) *= deweightFactor;
                    otherNoiseMatrix_ptr->col(otherIndex) *= deweightFactor;
                });
        }
        else
        {
...
bool satelliteGlitchReaction(RejectCallbackDetails rejectDetails)
{
    ...
    if (transitionRequired)
    {
        ...
        const string transitionLabel =
            "KF_STATE_REJECTION_RELAXATION/" +
            enum_to_string(kfKey.type) + "/" +
            (kfKey.str.empty() ? kfKey.Sat.id() : kfKey.str);
        if (!kfState.manualStateTransition(
                trace, kfState.time, F, Q, transitionLabel))
        {
            trace << "\nAUTHORITATIVE STATE RELAXATION TRANSACTION REJECTED"
...
bool relaxState(RejectCallbackDetails rejectDetails)
{
    ...
    {
        const string transitionLabel =
            "KF_POSTFIT_STATE_RELAXATION/" + description + "/" +
            enum_to_string(kfKey.type) + "/" +
            (kfKey.str.empty() ? kfKey.Sat.id() : kfKey.str) + "/" +
            (kfKey.num == 0 ? "NONE" : std::to_string(kfKey.num));
        if (!kfState.manualStateTransition(
                trace, kfState.time, F, Q, transitionLabel))
        {
            trace << "\nAUTHORITATIVE POSTFIT RELAXATION TRANSACTION REJECTED"
                  << " label=" << transitionLabel;
        }

        // Callback return controls the remaining rejection chain, not the
        // authoritative factor transaction.  A handled relaxation stops that
        // chain whether the transaction is provisional or already committed.
        return false;
    }
