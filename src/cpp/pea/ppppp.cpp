...
#pragma omp parallel for
    ...
    if (kfState.lsqRequired)
    {
        ...
        string suffix = "/LSQ";
        if (!kfState.leastSquareInitStates(trace, kfMeas, suffix, false, true))
        {
            trace << "\nPPP LEAST-SQUARE TRANSACTION REJECTED"
                  << " reason=" << kfState.lastFactorTransactionFailureReason;
            return false;
        }
    ...
    if (filterResult != KFFilterResult::COMMITTED)
    {
        trace << "\nPPP AUTHORITATIVE MEASUREMENT TRANSACTION REJECTED"
              << " result=" << static_cast<int>(filterResult)
              << " reason=" << kfState.lastFactorTransactionFailureReason;
        return false;
    }

    // The product prior is the exact post-QC, pre-measurement state used by
    // the committed factor, not the older pre-robust-loop state.
    captureZhangPppArFloatPrior(finalMeasurementPrior);
    ...
    {
        trace << "\nZHANG_PRODUCT_SEMANTICS status=NOT_AUTHORIZED"
              << " reason=ROBUST_QC_NOT_CONVERGED";
    }
