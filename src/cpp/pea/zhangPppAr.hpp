...
/** A temporal product endpoint is a physical product functional, independent
 * of whether that functional is emitted through a legacy tree coordinate or
 * a persistent satellite-potential coordinate.  HYBRID_STABLE must therefore
 * retain the same routine old/new snapshot coverage as the legacy frontend.
 * The coordinate-mode argument is intentionally not a gate; keeping it in
 * the signature makes regressions caused by mode-specific suppression
 * directly testable. */
inline bool zhangShouldCaptureRoutineTemporalProductSnapshot(
	bool houOsbLike,
	bool persistentHouCoordinate,
	bool fixedLagFactorCaptureShadow,
	bool structuralValid,
	bool alignmentEvaluable,
	bool stateDimensionMatches)
{
	(void)persistentHouCoordinate;
	return houOsbLike
		&& fixedLagFactorCaptureShadow
		&& structuralValid
		&& alignmentEvaluable
		&& stateDimensionMatches;
}
...
/** Complete L1/L2 satellite-minus-reference product marginal recovered from
 * the immutable raw-factor target window.  It is candidate evidence only: the
 * ambiguity solver must re-evaluate every integer on the current posterior. */
struct ZhangPersistentProductRelationMarginal
{
	bool valid = false;
	int relationRank = 0;
	int availableRows = 0;
	int stochasticRank = 0;
	int retainedIdentityEndpoints = 0;
	int missingIdentityEndpoints = 0;
	int phaseMismatchEndpoints = 0;
	// Exact accepted-measurement prefix that generated this cumulative
	// fixed-lag marginal.  These fields let the product Ledger distinguish a
	// genuinely new information increment from a second view of the same
	// heavily overlapping window.
	std::string factorWindowIdentity;
	std::vector<std::size_t> factorSequences;
	std::size_t factorMeasurementRows = 0;
	VectorXd mean;
	MatrixXd covariance;
	// Ordered exactly as mean/covariance.  Canonical queries retain the complete
	// 2*rank catalogue and mark missing rows false; callers must search only a
	// dual-frequency subset for which both signal rows are true.
	std::vector<bool> rowAvailable;
	std::string failureReason = "NOT_EVALUATED";
};

/** A retained endpoint may be used for a current canonical proposal only
 * while it still belongs to the same physical phase segment.  In particular,
 * an identity retained across a slip is historical evidence, not a current
 * integer relation. */
inline bool zhangPersistentProductSnapshotPhaseCompatible(
	const std::string& snapshotIdentity,
	const std::string& retainedPhaseSegment,
	const std::string& currentPhaseSegment)
{
	return !snapshotIdentity.empty() &&
		!retainedPhaseSegment.empty() &&
		!currentPhaseSegment.empty() &&
		currentPhaseSegment != "UNRESOLVED" &&
		retainedPhaseSegment == currentPhaseSegment;
}
...
/** Complete canonical satellite-minus-reference marginal, including target
 * directions absent from the current ambiguity-resolution posterior.  This is
 * proposal/confirmation evidence only: an unrepresentable direction must
 * enter the product Ledger lifecycle and must never be conditioned directly
 * into the current posterior. */
ZhangPersistentProductRelationMarginal
...
/** Lightweight scheduler probe.  A positive count means that the persistent
 * raw target window can be queried without first requiring a fresh network-WL
 * fix; it does not itself certify an integer relation. */
 ...
 * committed, QC-converged PPP transaction are defensively routed to the
 * stateless FLOAT-only writer above. */
