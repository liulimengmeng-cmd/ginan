...
env=os.environ.copy(); env.update(GINAN_AR_JOINT_NIS_GATE='1',LD_LIBRARY_PATH='/home/rx/.local/boost-1.82/lib',OMP_NUM_THREADS='1',OPENBLAS_NUM_THREADS='1',MKL_NUM_THREADS='1',GINAN_AR_SNAPSHOT_DIRECTORY=str(snaps),GINAN_AR_SNAPSHOT_TIMES='|'+'|'.join(times)+'|')
...
with (r/f'{mode}.stdout.log').open('x') as so,(r/f'{mode}.stderr.log').open('x') as se:
    ...
    (r/f'{mode}.started.json').open('x').write(json.dumps({'pid':p.pid,'argv':argv,'epoch_time_system':'GPST','snapshot_times':times,'changes_from_exp5':'joint gate enabled; RTS output disabled; stop at 2504 epochs','binary_sha256':sha(r/'pea_snapshot'),'config_sha256':sha(cfg),'nice':19,'address_space_limit_MiB':2200,'existing_other_pea_pid':522736,'started_unix':start},indent=2))
    rc=p.wait()
