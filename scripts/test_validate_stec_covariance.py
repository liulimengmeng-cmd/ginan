...
class StecCovarianceValidatorTests(unittest.TestCase):
    def test_accepts_complete_positive_definite_epoch(self) -> None:
        path = write_case(
            """META,2323,12345.5,OK,2,3,0,FILTER_POSTERIOR_AFTER_AR_PSEUDOOBS_SUBMITTED_UNVERIFIED,1,5,2,1,LAMBDA_ALT,0.9999,3,RESOLVED_RATIO_ACCEPTED,2,2,0.99995,1.25,5,4
...
"""
    ...
    def test_rejects_incomplete_or_indefinite_epoch(self) -> None:
        path = write_case(
            """META,2323,12345.5,OK,2,3,0,FILTER_POSTERIOR_NO_EPOCH_AR,0,0,0,0,OFF,0.9999,3,NOT_RUN,0,0,-1,-1,-1,-1
...
"""
    ...
    def test_rejects_writer_skip_status(self) -> None:
        path = write_case(
            "META,2323,12345.5,STATE_LIMIT_EXCEEDED,600,0,0,FILTER_POSTERIOR_NO_EPOCH_AR,0,0,0,0,OFF,0.9999,3,NOT_RUN,0,0,-1,-1,-1,-1\n"
        )
        report = validate_file(path)
        self.assertFalse(report["all_valid"])
    ...
    def test_rejects_inconsistent_ar_evidence(self) -> None:
        path = write_case(
            ...
            "FILTER_POSTERIOR_NO_EPOCH_AR,0,5,2,1,LAMBDA_ALT,0.9999,3,"
    ...
    def test_reports_malformed_short_record_without_index_error(self) -> None:
        path = write_case(
            "META,2323,12345.5,STATE_LIMIT_EXCEEDED,600,0,0,"
            "FILTER_POSTERIOR_NO_EPOCH_AR,0,0,0,0,OFF,0.9999,3,"
            "NOT_RUN,0,0,-1,-1,-1,-1\nSTATE\n"
        )
