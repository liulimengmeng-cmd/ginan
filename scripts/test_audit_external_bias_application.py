...
class ExternalBiasApplicationAuditTest(unittest.TestCase):
    def test_counts_conditioning_and_duplicate_keys(self) -> None:
        ...
        persistent = (
            "PPP_EXTERNAL_BIAS_APPLICATION type=PHASE time=2024-07-17 00:00:00.000 "
            "receiver=GRAZ satellite=G02 signal=L2W bias_found=1 bias_m=0.3 "
            "product_variance_m2=0.025 applied_variance_m2=0 variance_mode=PERSISTENT_STATE "
            "state_prior_variance_m2=0.025 state_prior_initialised=1"
        )
        with tempfile.TemporaryDirectory() as temporary:
            trace = Path(temporary) / "Network.trace"
            trace.write_text(
                "\n".join([phase, phase, code, persistent, "PPP_EXTERNAL_BIAS_APPLICATION bad"]),
                encoding="utf-8",
            )
            report = audit_trace(trace)

        self.assertEqual(report["marker_count"], 4)
        self.assertEqual(report["unique_application_key_count"], 3)
        ...
        self.assertEqual(report["nonzero_state_prior_variance_count"], 1)
        self.assertEqual(report["state_prior_initialisation_count"], 1)
        self.assertEqual(
            report["counts_by_type_signal_mode"],
            {
                "CODE/L1C/PER_EPOCH": 1,
                "PHASE/L1C/CONDITIONED": 2,
                "PHASE/L2W/PERSISTENT_STATE": 1,
            },
        )
