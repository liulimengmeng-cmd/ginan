...
def main():
 ...
 for case in s['cases']:
  receipt=json.loads((W/(case['case']+'.result.json')).read_text());assert receipt['exit_code']==0
  ...
  diag=rec['ZHANG_USER_DIAGNOSTIC'];times=[r['time'] for r in diag];assert times==expected,(case['case'],len(times))
 ...
 report=['# R48 用户端 PPP-AR 配对试验','',f"窗口：2024-07-17 00:03:00–00:24:30，44历元。五个独立用户站、10个用例均正常完成。",'', '采用自由位置估计、CANONICAL_USER_IF_WL_L1、同一 PRODUCT_FIXED 前缀与完整协方差。两组仅改变 AR 模式和整数反馈开关；成功率0.99、ratio 3、目标 perr≤1e-3、NIS alpha=1e-6 保持原值。','', '|站点|WL通过历元|WL最大选中数|L1通过历元|实际新增固定|FLOAT 3D RMS(m)|AR 3D RMS(m)|','|---|---:|---:|---:|---:|---:|---:|']
 for p in pairs:report.append(f"|{p['station']}|{p['wl_passed_epochs']}|{p['wl_selected_peak']}|{p['l1_passed_epochs']}|{p['newly_fixed_total']}|{p['float_3d_rms_m']:.4f}|{p['ar_3d_rms_m']:.4f}|")
 report += ['', '定位误差相对 IGS 周解 SINEX 的先验坐标，已核对 POS 参考坐标一致。用户站未进入 R48 180站产品估计，但参考坐标参与了用户解的初始化；位置自由估计，先验标准差100 m。全窗 RMS 包含冷启动，不能替代长期稳态精度。', '', '上游仍有5次 DGEMV 告警；本次10个用户用例均无 DGEMV、事务回滚或致命错误。上游产品的最终数值验收仍未通过。', '', 'WL选中或 feedback=1 不代表用户 IF/L1 整数约束已成功提交；以实际新增固定、保持秩和 L1 门控共同判定。没有独立整数真值，不报告“误固定率为零”。', '', '预检记录：首次冻结请求被拒绝，原因是00:00–00:02:30没有 PRODUCT_FIXED；初次用户运行缺少IF联合协方差回调需要的因子缓冲区；第二次因关闭 STEC 设计列导致 IF 组合未构造。正式用例已启用因子记录并恢复 STEC 设计列，两组一致。所有失败产物保留。', '', '产物：per_epoch.csv（440行）、stage_gates.csv、named_targets.csv、paired_summary.csv、summary.json、output_sha256.json。输入数据只引用与哈希，不复制。']
 if not result['successful_full_user_ar_demonstrated']:report[2:2]=['结论：用户产品接入链路已跑通，但本次未实现完整用户端 PPP-AR 固定，也未证明定位改善。','']
