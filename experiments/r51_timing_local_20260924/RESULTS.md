...
## Full local continuation started for cross-host comparison

On 2026-09-24, the same local binary SHA-256 `986078c9084b40832dfbbaed0ec66f9a2fe6da83f5a800a079385ec4f56cbc16` was launched in root-owned tmux session `r51_local_ar1h_20260924` from the unchanged 01:59:30 checkpoint. The isolated ext4 output root is `/home/rx/GINAN/local_timing/r51_20260924_ledger_snapshot_ar1h_01/`; the intended 120 AR epochs are 02:00:00 through 02:59:30. The run has a 14-day timeout rather than the earlier two-hour diagnostic timeout. A separate `r51_local_watch_20260924` session writes `process_memory.jsonl` every 10 seconds and terminates only sustained process swap >= 2 GiB or system available memory <= 1 GiB for 12 consecutive samples. The pre-existing outputs and the two PEA jobs on `comput` were not modified.

The first 02:00:00 product bundle committed with `ar:true` and had byte-identical `SHA256SUMS` and `COMMIT.json` to the prior same-host row-snapshot run. Both local and `comput` demand-driven runs reported 2,620 candidates and 2,080 newly fixed rows in `ZHANG_AR_SUMMARY`; these counts are not a current-epoch loop fixing rate. The cross-host first-epoch snapshot is:

| Metric | Local Ubuntu-22.04-G, new binary | `comput`, older demand-driven binary |
| --- | ---: | ---: |
| `AR_TOTAL` | 924.181 s | 4,046.139 s |
| Process HWM after `AR_CALL` | 12,091,924 KiB (11.53 GiB) | 20,126,224 KiB (19.19 GiB) |
| Process swap after `AR_CALL` | 0 KiB | 0 KiB |
| Committed 02:00:00 products | yes | yes |

The six configuration hashes and checkpoint-bundle hash matched the server run manifest. However, the cross-host `COMMIT.json` numerical root and product `SHA256SUMS` differ, despite matching first-epoch summary counts. The server executable was built from `9814fd1f`, while the local executable includes later instrumentation and ledger changes; host floating-point execution and input-file content also remain possible contributors. Thus cross-host product differences cannot be attributed to this ledger optimization without a same-binary, same-input control. The server presents 28 virtual Xeon Silver 4208 CPUs and was running two PEA jobs; local WSL presents 10 Ryzen 5 9600X logical CPUs. Per-process CPU use was about 1–1.5 cores, so a 28-to-10 core count comparison does not explain per-epoch throughput. As of this snapshot, the full local hour had **not** completed; the run continued into the next epoch.
