...
commit=git('rev-parse','HEAD').strip();assert git('rev-parse','refs/remotes/fork/codex/r50-ratio-only').strip()==commit
...
(f/'evidence/runtime.json').write_text(json.dumps({'source_commit':commit,'ldd':subprocess.check_output(['ldd',str(f/'bin/pea')],text=True),'policy':spec['r50_policy']},indent=2)+'\n')
(f/'SHA256SUMS').write_text('\n'.join(f'{sha(p)}  {p.relative_to(f).as_posix()}' for p in sorted(f.rglob('*')) if p.is_file())+'\n')
result={'frozen_root':str(f),'source_commit':commit,'binary_sha256':sha(f/'bin/pea'),'tests_passed':True,'source_matches_build':True,'input_data_copied':False,'frozen_at':datetime.datetime.now().astimezone().isoformat()}
