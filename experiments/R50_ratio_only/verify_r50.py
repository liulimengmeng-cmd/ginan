...
def preflight():
 ...
 (W/'r50_input_preflight.json').write_text(json.dumps(out,indent=2)+'\n')
...
def final(status):
 ...
 try:
  ...
  if [e['epoch'] for e in epochs]!=list(range(1405209600,1405211401,30)):result['failures'].append('EPOCH_COVERAGE')
  ...
  if result['checkpoint_bundles']!=61:result['failures'].append('CHECKPOINT_COVERAGE')
  ...
  (W/'R50_result_summary.md').write_text('\n'.join(lines)+'\n')
